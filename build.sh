...
build_kernel() {
    ...
    echo "-----------------------------------------------"
    make ${MAKE_ARGS} -j$CORES exynos2200_r11s_defconfig || abort
...
build_boot() {
    ...
    cp -a arch/arm64/boot/Image build/out/$MODEL
...
build_dtb() {
    ...
    ./toolchain/mkdtimg cfg_create build/out/$MODEL/dtb.img build/dtconfigs/s5e9925.cfg -d arch/arm64/boot/dts/exynos || abort 

    echo "-----------------------------------------------"
    echo "Building DTBO image..."
    ./toolchain/mkdtimg cfg_create build/out/$MODEL/dtbo.img build/dtconfigs/$MODEL.cfg -d arch/arm64/boot/dts/samsung/$MODEL || abort

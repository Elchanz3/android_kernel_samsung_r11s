...
rm -rf build/out/r11s
mkdir -p build/out/r11s/zip/files
mkdir -p build/out/r11s/zip/META-INF/com/google/android
...
build_boot() {
    ...
    cp -a arch/arm64/boot/Image build/out/r11s

    if [ -z "$RECOVERY" ]; then			   
    echo "-----------------------------------------------"
    echo "Building boot.img RAMDisk..."
    mkdir -p build/out/r11s/boot_ramdisk00

    # Copy common files for boot.img's RAMDisk
    cp -a build/ramdisk/boot/boot_ramdisk00 build/out/r11s
    
    # Copy device build.prop file for boot.img's RAMDisk
    cp -a build/ramdisk/boot/device/r11s/* build/out/r11s/boot_ramdisk00/system/etc/ramdisk

    pushd build/out/r11s/boot_ramdisk00 > /dev/null
    ...
    OUTPUT_FILE=build/out/r11s/boot.img
    RAMDISK_00=build/out/r11s/boot_ramdisk
    KERNEL=build/out/r11s/Image
    HEADER_VERSION=4
    OS_VERSION=15.0.0
...
build_dtb() {
    ...
    ./toolchain/mkdtimg cfg_create build/out/r11s/dtb.img build/dtconfigs/s5e9925.cfg -d arch/arm64/boot/dts/exynos || abort 

    echo "-----------------------------------------------"
    echo "Building DTBO image..."
    ./toolchain/mkdtimg cfg_create build/out/r11s/dtbo.img build/dtconfigs/r11s.cfg -d arch/arm64/boot/dts/samsung/r11s || abort
...
build_modules() {
    ...
    mkdir -p build/out/r11s/modules_dlkm
    ...
    for FILENAME in $FILENAMES; do
        FILE=$(find $MODULES_FOLDER -type f -name "$FILENAME")
        cp "$FILE" "build/out/r11s/modules_dlkm"
        rm -f "$FILE"
    done

    # Now we run depmod to update the dep/softdep files
    # For this we need the kernel version
    KERNEL_DIR_PATH=$(find "$MODULES_FOLDER/lib/modules" -maxdepth 1 -type d -name "5.10*") || abort
    KERNEL_VERSION=$(basename $KERNEL_DIR_PATH) || abort

    # And finally depmod itself
    depmod -a -b $MODULES_FOLDER $KERNEL_VERSION || abort
...
build_vendor_boot() {
    ...
    pushd build/out/r11s/vendor_ramdisk00 > /dev/null
    find . ! -name . | LC_ALL=C sort | cpio -o -H newc -R root:root | lz4 -l > ../vendor_ramdisk || abort
    popd > /dev/null

    pushd build/out/r11s/modules > /dev/null
...
build_zip() {
    echo "-----------------------------------------------"
    echo "Building zip..."
    cp build/out/r11s/boot.img build/out/r11s/zip/files/boot.img
    cp build/out/r11s/vendor_boot.img build/out/r11s/zip/files/vendor_boot.img
    cp build/out/r11s/dtbo.img build/out/r11s/zip/files/dtbo.img

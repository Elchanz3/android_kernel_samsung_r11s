...
/*
 *  Sound Machine Driver for tfa AMPs on exynos9925
 *
...
static struct _drvdata exynos9925_drvdata;
...
#if 0
void tfa_i2c_fail_log(const char *suffix)
{
	pr_info("%s(%s)\n", __func__, suffix);
#if IS_ENABLED(CONFIG_SEC_ABC)
...
void tfa9878_amp_fail_event(const char *suffix)
{
	pr_info("%s(%s)\n", __func__, suffix);
#if IS_ENABLED(CONFIG_SEC_ABC)
#if IS_ENABLED(CONFIG_SEC_FACTORY)
...
static int exynos9925_uaif1_init(struct snd_soc_pcm_runtime *rtd)
{
...
static int exynos9925_dai_ops_hw_params(struct snd_pcm_substream *substream,
...
static const struct snd_soc_ops uaif_ops = {
	.hw_params = exynos9925_dai_ops_hw_params,
};
...
static int exynos9925_set_bias_level(struct snd_soc_card *card,
...
static int exynos9925_set_bias_level_post(struct snd_soc_card *card,
					 struct snd_soc_dapm_context *dapm,
					 enum snd_soc_bias_level level)
{
	dev_dbg(card->dev, "%s: %d\n", __func__, level);
...
static int exynos9925_late_probe(struct snd_soc_card *card)
{
...
static const struct snd_soc_dapm_widget exynos9925_supply_widgets[] = {
...
static int exynos9925_probe(struct snd_soc_card *card)
{
	struct _drvdata *drvdata = card->drvdata;
	int i;

	for (i = 0; i < ARRAY_SIZE(exynos9925_supply_widgets); i++) {
		const struct snd_soc_dapm_widget *w;
		struct regulator *r;

		w = &exynos9925_supply_widgets[i];
		switch (w->id) {
	...
	drvdata->ws = wakeup_source_register(NULL, "exynos9925-sound");

	return 0;
}

static int exynos9925_remove(struct snd_soc_card *card)
{
	struct _drvdata *drvdata = card->drvdata;
...
static struct snd_soc_dai_link exynos9925_dai[100] = {
	{
		.name = "RDMA0",
		.stream_name = "RDMA0",
		.dynamic = 1,
...
static int exynos9925_dmic1(struct snd_soc_dapm_widget *w,
...
static int exynos9925_dmic2(struct snd_soc_dapm_widget *w,
			struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_card *card = w->dapm->card;
...
static int exynos9925_dmic3(struct snd_soc_dapm_widget *w,
			struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_card *card = w->dapm->card;
...
static int exynos9925_l_speaker(struct snd_soc_dapm_widget *w,
			struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_card *card = w->dapm->card;
...
static int exynos9925_r_speaker(struct snd_soc_dapm_widget *w,
			struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_card *card = w->dapm->card;
...
static int exynos9925_bt_mic(struct snd_soc_dapm_widget *w,
			struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_card *card = w->dapm->card;
...
static int exynos9925_bt_out(struct snd_soc_dapm_widget *w,
			struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_card *card = w->dapm->card;
...
static int exynos9925_haptic_out(struct snd_soc_dapm_widget *w,
			struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_card *card = w->dapm->card;
...
static int exynos9925_usb_mic(struct snd_soc_dapm_widget *w,
			struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_card *card = w->dapm->card;
...
static int exynos9925_usb_out(struct snd_soc_dapm_widget *w,
			struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_card *card = w->dapm->card;
...
static int exynos9925_fwd_mic(struct snd_soc_dapm_widget *w,
			struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_card *card = w->dapm->card;
...
static int exynos9925_fwd_out(struct snd_soc_dapm_widget *w,
			struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_card *card = w->dapm->card;
...
{
	struct _drvdata *drvdata = &exynos9925_drvdata;
...
static const struct snd_kcontrol_new exynos9925_controls[] = {
...
static const struct snd_soc_dapm_widget exynos9925_widgets[] = {
	...
	SND_SOC_DAPM_MIC("DMIC1", exynos9925_dmic1),
	SND_SOC_DAPM_MIC("DMIC2", exynos9925_dmic2),
	SND_SOC_DAPM_MIC("DMIC3", exynos9925_dmic3),
	SND_SOC_DAPM_SPK("RECEIVER", exynos9925_l_speaker),
	SND_SOC_DAPM_SPK("SPEAKER", exynos9925_r_speaker),
	SND_SOC_DAPM_MIC("SPEAKER FB", NULL),
	SND_SOC_DAPM_MIC("BLUETOOTH MIC", exynos9925_bt_mic),
	SND_SOC_DAPM_SPK("BLUETOOTH SPK", exynos9925_bt_out),
	SND_SOC_DAPM_SPK("HAPTIC SPK", exynos9925_haptic_out),
	SND_SOC_DAPM_MIC("USB MIC", exynos9925_usb_mic),
	SND_SOC_DAPM_SPK("USB SPK", exynos9925_usb_out),
	SND_SOC_DAPM_MIC("FWD MIC", exynos9925_fwd_mic),
	SND_SOC_DAPM_SPK("FWD SPK", exynos9925_fwd_out),
	SND_SOC_DAPM_OUTPUT("VTS Virtual Output"),
	SND_SOC_DAPM_MUX("VTS Virtual Output Mux", SND_SOC_NOPM, 0, 0,
			&vts_output_mux[0]),
};

static const struct snd_soc_dapm_route exynos9925_routes[] = {
...
static struct snd_soc_card exynos9925_tfa = {
	.name = "Exynos9925-Tfa",
	.owner = THIS_MODULE,
	.dai_link = exynos9925_dai,
	.num_links = ARRAY_SIZE(exynos9925_dai),

	.probe = exynos9925_probe,
	.late_probe = exynos9925_late_probe,
	.remove = exynos9925_remove,

	.controls = exynos9925_controls,
	.num_controls = ARRAY_SIZE(exynos9925_controls),
	.dapm_widgets = exynos9925_widgets,
	.num_dapm_widgets = ARRAY_SIZE(exynos9925_widgets),
	.dapm_routes = exynos9925_routes,
	.num_dapm_routes = ARRAY_SIZE(exynos9925_routes),

	.set_bias_level = exynos9925_set_bias_level,
	.set_bias_level_post = exynos9925_set_bias_level_post,

	.drvdata = (void *)&exynos9925_drvdata,
...
static void exynos9925_register_card_work_func(struct work_struct *work)
{
	struct snd_soc_card *card = &exynos9925_tfa;
	int retry = 10, ret;
...
DECLARE_WORK(exynos9925_register_card_work, exynos9925_register_card_work_func);

static int exynos9925_sound_probe(struct platform_device *pdev)
{
	struct snd_soc_card *card = &exynos9925_tfa;
	struct _drvdata *drvdata = card->drvdata;
	...
	schedule_work(&exynos9925_register_card_work);

	return ret;
}

static int exynos9925_sound_remove(struct platform_device *pdev)
{
	...
	for (dai_link = exynos9925_dai; dai_link - exynos9925_dai <
			ARRAY_SIZE(exynos9925_dai); dai_link++) {
...
static const struct of_device_id exynos9925_sound_of_match[] = {
	{ .compatible = "samsung,exynos9925-tfa", },
	{},
};
MODULE_DEVICE_TABLE(of, exynos9925_sound_of_match);
#endif /* CONFIG_OF */

static struct platform_driver exynos9925_sound_driver = {
	.driver		= {
		.name	= "exynos9925-tfa",
		.owner	= THIS_MODULE,
		.pm = &snd_soc_pm_ops,
		.of_match_table = of_match_ptr(exynos9925_sound_of_match),
	},

	.probe		= exynos9925_sound_probe,
	.remove		= exynos9925_sound_remove,
};

module_platform_driver(exynos9925_sound_driver);

MODULE_DESCRIPTION("ALSA SoC Eynos9925-tfa Sound Driver");
MODULE_LICENSE("GPL");
MODULE_ALIAS("platform:exynos9925-tfa");
